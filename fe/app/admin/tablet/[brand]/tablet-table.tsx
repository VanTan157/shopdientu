...
const LaptopTable = ({ tablets }: { tablets: ITablet[] }) => {
  ...
  const filteredTablets = tablets.filter((tablet) => {
    ...
    const matchesStatus =
      filterStatus === "Tất cả" ||
      tablet.isAvailable.toString() === filterStatus;
    const matchesPromotion =
      filterPromotion === "Tất cả" ||
      tablet.isPromotion.toString() === filterPromotion;
        ...
        <Table className="w-full h-full">
          ...
          <TableBody>
            {filteredTablets.map((tablet, index) => (
              <TableRow key={tablet._id}>
                <TableCell className="text-center">{index + 1}</TableCell>
                <TableCell className="text-center">{tablet.name}</TableCell>
                ...
                <TableCell className="flex gap-2 items-center justify-center">
                  <ViewLaptopDetail tablet={tablet}>
